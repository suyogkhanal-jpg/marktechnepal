...
export function PrintReceipt({ receipts }: PrintReceiptProps) {
  ...
  return (
    ...
    >
      ...
      {/* Header Section */}
      <div className="flex items-start justify-between" style={{ marginBottom: '1.5mm' }}>
        {/* Logo */}
        <div style={{ width: '14mm', flexShrink: 0 }}>
          <img src={marktechLogo} alt="MarkTech Nepal" style={{ height: '12mm', width: 'auto' }} />
        </div>
        
        {/* Title */}
        <div className="text-center flex-1" style={{ padding: '0 2mm' }}>
          <h1 style={{ ...headerFont, fontSize: '20pt', margin: 0, lineHeight: 1.1 }}>
            MARKTECH NEPAL
          </h1>
          <h2 style={{ 
            ...labelFont, 
        ...
        {/* Address & Contact */}
        <div className="text-right" style={{ fontSize: '7pt', lineHeight: 1.2, flexShrink: 0, width: '38mm' }}>
          ...
          <div className="flex items-center justify-end" style={{ gap: '1mm' }}>
            <svg style={{ width: '7px', height: '7px' }} viewBox="0 0 24 24" fill="#25D366">
            ...
            <svg style={{ width: '7px', height: '7px' }} viewBox="0 0 24 24" fill="#7360F2">
      ...
      <div className="flex justify-between items-center" style={{ marginBottom: '1.5mm', fontSize: '9pt' }}>
      ...
      {/* Customer Info */}
      <div style={{ marginBottom: '1.5mm', fontSize: '9pt' }}>
        <div className="flex items-baseline" style={{ marginBottom: '1mm' }}>
      ...
      >
        <thead>
          <tr style={{ backgroundColor: '#f0f0f0' }}>
            <th style={{ ...labelFont, border: '1px solid #000', padding: '1mm', textAlign: 'center', width: '6mm' }}>S.N</th>
            <th style={{ ...labelFont, border: '1px solid #000', padding: '1mm', textAlign: 'center', width: '28mm' }}>Particulars</th>
            <th style={{ ...labelFont, border: '1px solid #000', padding: '1mm', textAlign: 'center' }}>Problem</th>
            <th style={{ ...labelFont, border: '1px solid #000', padding: '1mm', textAlign: 'center', width: '28mm' }}>Model NO.</th>
            <th style={{ ...labelFont, border: '1px solid #000', padding: '1mm', textAlign: 'center', width: '28mm' }}>Serial No.</th>
          </tr>
        </thead>
        <tbody>
          {receipts.slice(0, 3).map((receipt, index) => (
            <tr key={receipt.id}>
              <td style={{ ...dataFont, border: '1px solid #000', padding: '1mm', textAlign: 'center', height: '8mm', verticalAlign: 'top' }}>
                {index + 1}
              </td>
              <td style={{ ...dataFont, border: '1px solid #000', padding: '1mm', verticalAlign: 'top', textTransform: 'capitalize', fontSize: '8pt' }}>
                {receipt.device_type}
                {receipt.accessories && <span style={{ display: 'block', fontSize: '6.5pt', color: '#555' }}>({receipt.accessories})</span>}
              </td>
              <td style={{ ...dataFont, border: '1px solid #000', padding: '1mm', verticalAlign: 'top', fontSize: '8pt' }}>
          ...
          {receipts.length < 3 && Array.from({ length: 3 - Math.min(receipts.length, 3) }).map((_, i) => (
            <tr key={`empty-${i}`}>
              <td style={{ border: '1px solid #000', padding: '1mm', height: '8mm' }}></td>
      ...
      {/* Tagline at Bottom - Bold and Larger */}
